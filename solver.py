...
def create_QP_instances(P,q,G,h,A,b):
    ...
    l = np.hstack([ -np.inf*np.ones(n_ineq),1.])
    u = np.hstack([np.zeros(n_ineq),1.])
    return P,q,osA, l,u
...
def BPQP(args,sign = -1):
    class BPQPmethod(Function):
        ...
        def forward(ctx, P,q):
            ...
            ctx.G = G.cpu().numpy()
            ctx.A = A.cpu().numpy()
            yy = torch.cat([torch.from_numpy(x_value).to(device).to(torch.float32), torch.from_numpy(y_value).to(device).to(torch.float32)],dim=0)
        ...
        def backward(ctx, grad_output):
            ...
            x_grad, _, _ = QP_OSQP_backward(x_star.detach().cpu().numpy(),lambda_star.detach().cpu().numpy(), P, G, A,grad_output)
            # x_grad = clip_grad(x_grad)
            try:
                x_grad = torch.from_numpy(x_grad).to(torch.float32).to(device)
            except:
                x_grad = None # No solution
            grads = (None, x_grad)
            return grads
...
class NNSolver(nn.Module):
    ...
    def forward(self, variance,x):
        x_in = torch.cat([variance,x.unsqueeze(1)],axis=1)
...
def grad_steps_all(Y,A,b,G,h, args):
    ...
    old_Y_step = 0
    with torch.no_grad():
        while (i == 0 or torch.max(torch.abs(A.mv(Y)-b)) > eps_converge or
               torch.max(G.mv(Y)-h) > eps_converge) and i < max_steps:
...
def PDIPM(args,sign = -1):
    class QPfunction_gpuFn(Function):
        ...
        def forward(ctx, Q,q):
            ...
            nineq = nz
            neq = 1
            initial_x, initial_lambs, initial_nu = (1/nz)*torch.ones(nz).to(device),torch.ones(nz).to(device),torch.zeros(1).to(device)
            ...
            x_star, lambs_star, nu_star, _, k_inverse = torch_qp_solver(Q, sign*q, _G, _h, _A, _b, initial_x, initial_lambs,
                                                             initial_nu, sigma=0.5, max_ite=500)
        ...
        def backward(ctx, grad_output):
            ...
            dzs,dls,dns = dys[:nz],dys[nz:(nz+nineq)],dys[-neq:]
...
def solve_grad_kkt_m(Q, G, h, A, x,lambs):
    ...
    L2 = torch.cat([torch.diag_embed(lambs).mm(G),torch.diag_embed(G.mv(x)-h),torch.zeros(nineq,neq).to(device)],dim = 1)
...
def torch_qp_solver(Q, q, G, h, A, b, x,lambs, nu, sigma,max_ite):
    ...
    for _ in range(max_ite):
        ...
        while (torch.norm(kkt_trail)>(1-0.1*s)*torch.norm(kkt)):
            ...
            lambs_trail = lambs + s * delta_y[nz:(nz + nineq)]
            nu_trail = nu + s * delta_y[(nz + nineq):]
            kkt_trail = solve_kkt_r(Q=Q, q=q, G=G, h=h, A=A, b=b, x=x_trail, lambs=lambs_trail, nu=nu_trail,
                                    elips=elips) # Last KKT
        x = x + s * delta_y[:nz]
        ...
        ita = -lambs.dot((G.mv(x)-h))/nineq
        ita_store.append(ita)
        if (ita < 1e-3) and torch.sqrt(torch.norm(delta_y[:nz],p=2)**2+torch.norm(delta_y[(nz + nineq):],p=2)**2)< 1e-3:
            break
